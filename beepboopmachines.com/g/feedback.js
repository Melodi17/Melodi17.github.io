...
function evaluate() {
    ...
    if (window.graphTitle == "My Graph") {
        labelerror = true;
        feedback.push({severity: 100, msg: "Don't forget to add a clear title that describes the relationship in the graph."});
    }
    if (window.graphXAxis == "X-Axis") {
        labelerror = true;
        feedback.push({severity: 100, msg: "Don't forget to add a label describing the quantity and units on the X-Axis"});
    }
    if (window.graphYAxis == "Y-Axis") {
        labelerror = true;
        feedback.push({severity: 100, msg: "Don't forget to add a label describing the quantity and units on the Y-Axis"});
    }
    if (window.graphXAxisUnits == "units") {
        labelerror = true;
        feedback.push({severity: 100, msg: "Don't forget to include the units for the horizontal axis"});
    }
    if (window.graphYAxisUnits == "units") {
        labelerror = true;
        feedback.push({severity: 100, msg: "Don't forget to include the units for the vertical axis"});
    }
    ...
    if (labelerror == true) {
        feedback.push({severity: 0, msg: "You can edit the labels on the graph by clicking on them"});
    }
...
/*    var msg = "Some uncertainties on the dependent variable are so large that they cross over adjacent data. Consider ways to reduce the measurement uncertainty or increase the separation of the values you collect for your dependent variable: ";
    ...
    var s = "";
    if (feedback.length != 1) { s = "s";}
    div.innerHTML = "<p style=\"color:black\">You have " +feedback.length+" message" + s + ":</p>";
